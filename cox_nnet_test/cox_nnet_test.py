...
def trainCoxMlp(x_train, ytime_train, ystatus_train, 
model_params = dict(L2_reg=numpy.exp(-1), node_map=None, input_split=None), 
search_params = dict(method = "nesterov", learning_rate=0.010, momentum=0.9, lr_decay = 0.5, lr_growth = 1.1, eval_step = 53, max_iter=10000, stop_threshold=0.995, patience=2000, patience_incr=1.5, rand_seed = 123), verbose=True):

    L2_reg = model_params['L2_reg']
    node_map = model_params['node_map']
    input_split = model_params['input_split']
    ...
    lr_decay = search_params['lr_decay'] if "lr_decay" in search_params else 0.5
    lr_growth = search_params['lr_growth'] if "lr_growth" in search_params else 1.1
    eval_step = search_params['eval_step'] if "eval_step" in search_params else 53 
    # rho = createSharedDataset(float(search_params['rho'])) if "rho" in search_params else createSharedDataset(0.95)
    # epsilon = createSharedDataset(float(search_params['epsilon'])) if "epsilon" in search_params else createSharedDataset(1e-6)
    
    max_iter = search_params['max_iter']
    stop_threshold = search_params['stop_threshold']
    patience = search_params['patience']
    patience_incr = search_params['patience_incr']
    rand_seed = search_params['rand_seed']
...
def crossValidate(x_train, ytime_train, ystatus_train, 
model_params = dict(node_map = None, input_split = None, L2_reg = numpy.exp(-1)),
search_params = dict(learning_rate=0.01, max_iter=10000, stop_threshold=0.995, patience=2000, patience_incr=2, rand_seed = 123),
cv_params = dict(cv_seed=1, n_folds=10, cv_metric = "loglikelihood"), verbose=True):

    cv_seed = cv_params['cv_seed']
    n_folds = cv_params['n_folds']
    cv_metric = cv_params['cv_metric']
    N_train = ytime_train.shape[0]
...
def L2CVSearch(x_train, ytime_train, ystatus_train, 
model_params = dict(node_map = None, input_split = None),
search_params = dict(learning_rate=0.01, max_iter=10000, stop_threshold=0.995, patience=2000, patience_incr=1.5, rand_seed = 123),
cv_params = dict(cv_seed=1, n_folds=10, cv_metric = "loglikelihood", search_iters = 5, L2_range = [-7,1]), verbose=True):
    
    L2_range = cv_params['L2_range']
    n_folds = cv_params['n_folds']
    search_iters = cv_params['search_iters']
...
def L2CVProfile(x_train, ytime_train, ystatus_train, 
model_params = dict(node_map = None, input_split = None, L2_reg = numpy.exp(-1)),
search_params = dict(learning_rate=0.01, max_iter=10000, stop_threshold=0.995, patience=2000, patience_incr=1.5, rand_seed = 123),
cv_params = dict(cv_seed=1, n_folds=10, cv_metric = "loglikelihood", search_iters = 5, L2_range = [-6,-5,-4,-3,-2,-1,0]), verbose=True):
